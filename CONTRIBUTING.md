...
Prefer direct `assert!` / `assert!(!...)` assertions over tables of positional
booleans that require readers to map each value back to a predicate. Related
checks can share a test; separate test functions are not a goal by themselves.
Keep tables and loops when their inputs and expected results are easy to read.
