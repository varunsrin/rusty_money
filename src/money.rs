...
mod tests {
    ...
    fn money_major_minor() {
        let major_usd = Money::from_major(10, iso::USD);
        let minor_usd = Money::from_minor(1000, iso::USD);
        assert_eq!(major_usd, minor_usd);
    }

    #[test]
    fn money_from_string_parses_correctly() {
        let expected_money = Money::from_minor(2999, iso::GBP);
        let money = Money::from_str("29.99", iso::GBP).unwrap();
        assert_eq!(money, expected_money);
    }

    #[test]
    fn money_from_string_parses_signs() {
        let expected_money = Money::from_minor(-300, iso::GBP);
        let money = Money::from_str("-3", iso::GBP).unwrap();
        assert_eq!(money, expected_money);
    ...
    fn money_from_string_ignores_separators() {
        let expected_money = Money::from_minor(100000000, iso::GBP);
        let money = Money::from_str("1,000,000", iso::GBP).unwrap();
        assert_eq!(money, expected_money);
    }

    #[test]
    fn money_from_string_decimal_sanity() {
        let money = Money::from_str("1,00.00", iso::GBP);
        assert_eq!(money.unwrap_err(), MoneyError::InvalidAmount);

        let money = Money::from_str("1.00,00", iso::EUR);
        assert_eq!(money.unwrap_err(), MoneyError::InvalidAmount);

        let money = Money::from_str("1.00.000,00", iso::EUR);
        assert_eq!(money.unwrap_err(), MoneyError::InvalidAmount);

        let money = Money::from_str("1.00.000.000,00", iso::EUR);
        assert_eq!(money.unwrap_err(), MoneyError::InvalidAmount);

        let money = Money::from_str("1,00.00", iso::INR);
        assert_eq!(money.unwrap_err(), MoneyError::InvalidAmount);

        let money = Money::from_str("1.000.000.00", iso::INR);
        assert_eq!(money.unwrap_err(), MoneyError::InvalidAmount);
    }
    ...
    fn money_from_string_parse_errs() {
        ...
        let money = Money::from_str("1.0000,000", iso::GBP);
        assert_eq!(money.unwrap_err(), MoneyError::InvalidAmount);
        ...
        let exponent_separator_only = Money::from_str(",", iso::GBP);
        let amount_separator_only = Money::from_str(".", iso::GBP);
        let both_separators = Money::from_str(",,.", iso::GBP);
        assert_eq!(
    ...
    fn money_format_rounds_exponent() {
        ...
        let money = Money::from_str("19.9999", iso::USD).unwrap();
        assert_eq!("$20.00", format!("{}", money));

        // // 29.111 rounds to 29.11 for USD
        let money = Money::from_str("29.111", iso::USD).unwrap();
        assert_eq!("$29.11", format!("{}", money));

        // // 39.1155 rounds to 39.116 for BHD
        let money = Money::from_str("39.1155", iso::BHD).unwrap();
    ...
    fn money_addition_and_subtraction() {
        // Addition
        assert_eq!(
            Money::from_major(2, iso::USD),
            Money::from_major(1, iso::USD) + Money::from_major(1, iso::USD)
        );
        // Subtraction
        assert_eq!(
            Money::from_major(0, iso::USD),
            Money::from_major(1, iso::USD) - Money::from_major(1, iso::USD)
        );
    }
    ...
    fn money_add_assign_panics_on_different_currencies() {
        let mut money = Money::from_minor(100, iso::USD);
        money += Money::from_minor(100, iso::GBP);
    }
    ...
    fn money_multiplication_and_division() {
        ...
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Money::from_minor(100, iso::USD) * 2
        );
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Money::from_minor(-100, iso::USD) * -2
        ...
        assert_eq!(
            Money::from_minor(200, iso::USD),
            -2 * Money::from_minor(-100, iso::USD)
        );

        // Multiplication decimal
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Money::from_minor(100, iso::USD) * Decimal::new(2, 0)
        );
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Money::from_minor(-100, iso::USD) * Decimal::new(-2, 0)
        ...
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Decimal::new(-2, 0) * Money::from_minor(-100, iso::USD)
        );
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Money::from_minor(400, iso::USD) * Decimal::new(5, 1)
        );

        // Division integer
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Money::from_minor(400, iso::USD) / 2
        );
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Money::from_minor(-400, iso::USD) / -2
        ...
        assert_eq!(
            ...
            -1 / Money::from_minor(-200, iso::USD)
        );
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Money::from_minor(-200, iso::USD) / -1
        );

        // Division decimal
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Money::from_minor(400, iso::USD) / Decimal::new(2, 0)
        );
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Money::from_minor(-400, iso::USD) / Decimal::new(-2, 0)
        ...
        assert_eq!(
            Money::from_minor(50, iso::USD),
            Decimal::new(-1, 0) / Money::from_minor(-200, iso::USD)
        );
        assert_eq!(
            Money::from_minor(200, iso::USD),
            Money::from_minor(-200, iso::USD) / Decimal::new(-1, 0)
        );
        assert_eq!(
            Money::from_minor(400, iso::USD),
            Money::from_minor(-200, iso::USD) / Decimal::new(-5, 1)
        );

        //MulAssign integer
        let mut money = Money::from_minor(100, iso::USD);
        money *= 2;
        assert_eq!(Money::from_minor(200, iso::USD), money);
        ...
        let mut money = Money::from_minor(100, iso::USD);
        money /= -2;
        assert_eq!(Money::from_minor(-50, iso::USD), money);
    ...
    fn money_comparison() {
        ...
        assert_eq!(
            true,
            Money::from_minor(200, iso::USD) > Money::from_minor(100, iso::USD)
        );
        // Less Than
        assert_eq!(
            false,
        ...
        assert_eq!(
            true,
            Money::from_minor(100, iso::USD) == Money::from_minor(100, iso::USD)
        );
        assert_eq!(
            false,
        ...
        // is positive
        assert_eq!(true, Money::from_minor(100, iso::USD).is_positive());
        assert_eq!(false, Money::from_minor(0, iso::USD).is_positive());
        assert_eq!(false, Money::from_minor(-100, iso::USD).is_positive());
        // is zero
        assert_eq!(true, Money::from_minor(0, iso::USD).is_zero());
        assert_eq!(false, Money::from_minor(100, iso::USD).is_zero());
        assert_eq!(false, Money::from_minor(-100, iso::USD).is_zero());
        // is negative
        assert_eq!(true, Money::from_minor(-100, iso::USD).is_negative());
        assert_eq!(false, Money::from_minor(100, iso::USD).is_negative());
        assert_eq!(false, Money::from_minor(0, iso::USD).is_negative());
    }

    #[test]
    #[should_panic]
    fn money_ops_greater_than_panics_on_different_currencies() {
        assert!(Money::from_minor(100, iso::USD) < Money::from_minor(100, iso::GBP));
    }
    ...
    #[test]
    fn money_allocate() {
        let money = Money::from_minor(1_100, iso::USD);
        ...
        let monies = Money::from_minor(100, iso::USD).allocate(Vec::new());
        assert_eq!(monies.unwrap_err(), MoneyError::InvalidRatio);

        // Error if any ratio is zero
        let monies = Money::from_minor(100, iso::USD).allocate(vec![1, 0]);
        assert_eq!(monies.unwrap_err(), MoneyError::InvalidRatio);
    }
    ...
    fn money_allocate_to() {
        ...
        let monies = Money::from_minor(100, iso::USD).allocate_to(0);
        assert_eq!(monies.unwrap_err(), MoneyError::InvalidRatio);
    }

    #[test]
    fn money_fmt_separates_digits() {
        let usd = Money::from_minor(0, iso::USD); // Zero Dollars
        let expected_usd_fmt = "$0.00";
        assert_eq!(format!("{}", usd), expected_usd_fmt);

        let usd = Money::from_minor(10_000_000, iso::USD); // One Hundred Thousand Dollars
        ...
        let usd = Money::from_minor(100_000_000_000, iso::USD); // 1 Billion Dollars
        let expected_usd_fmt = "$1,000,000,000.00";
        assert_eq!(format!("{}", usd), expected_usd_fmt);

        let inr = Money::from_minor(10_000_000, iso::INR); // 1 Lakh Rupees
        let expected_inr_fmt = "₹1,00,000.00";
        assert_eq!(format!("{}", inr), expected_inr_fmt);

        let inr = Money::from_minor(-1_000_000_000, iso::INR); // - 1 Crore Rupees
    ...
    fn money_fmt_places_symbols_correctly() {
        let money = Money::from_minor(0, iso::USD);
        let expected_fmt = "$0.00";
        assert_eq!(format!("{}", money), expected_fmt);

        let money = Money::from_minor(0, iso::AED);
    ...
    fn money_fmt_uses_correct_separators() {
        let money = Money::from_minor(100_000, iso::EUR);
    ...
    fn money_precision_and_rounding() {
        let expected_money = Money::from_minor(667, iso::USD);
        let mut money = Money::from_minor(2_000, iso::USD);
        money /= 3;
        assert_eq!(money.round(2, Round::HalfEven), expected_money);

        let expected_money = Money::from_minor(6_667, iso::BHD);
        let mut money = Money::from_minor(20_000, iso::BHD);
        money /= 3;
