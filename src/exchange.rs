...
impl ExchangeRate {
    ...
    pub fn convert(self, amount: Money) -> Money {
        ...
        Money::new(converted_amount, self.to)
    }
}
...
mod tests {
    ...
    fn exchange_stores_rates() {
        let usd = Currency::find("USD".to_string());
        let eur = Currency::find("EUR".to_string());
        let rate = ExchangeRate::new(usd, eur, dec!(1.5));

        let mut exchange = Exchange::new();
        exchange.add_or_update_rate(rate.clone());
    ...
    fn rate_converts_money() {
        let rate = ExchangeRate::new(
            Currency::find("USD".to_string()),
            Currency::find("EUR".to_string()),
            dec!(1.5),
        );
